...
export default function UserMenu() {
    ...
    if (user) {
        Menu = <div className="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
            <div className="relative" ref={userMenuNode} >
                ...
                <div className={"absolute right-0 mt-2 py-2 w-48 bg-white rounded-md shadow-xl z-20 " + (userMenu ? 'block' : 'hidden')}>
                    ...
                    <button to="/" className="w-full text-left px-4 py-2 text-sm capitalize text-gray-700 hover:bg-indigo-700 hover:text-white">Sign Out</button>
                </div>
            </div>
        </div >
    } else {
        Menu = <div className="hidden md:flex items-center justify-end md:flex-1 lg:w-0">
            ...
            <NavLink to="/#" className="ml-8 whitespace-nowrap inline-flex items-center justify-center px-4 py-2 border border-transparent shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700">Sign up</NavLink>
        </div>
    }
    return (
        <>{Menu}</>
